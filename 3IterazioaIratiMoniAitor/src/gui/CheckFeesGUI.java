...
public class CheckFeesGUI extends JFrame {
	...
	private static CheckFeesGUI leihoa;
	// Code for JCalendar
	...
	{
		this.setSize(new Dimension(700, 558));
		...
		scrollPaneQueries.setBounds(40, 243, 299, 135);
		...
		btnSetResult.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				String emaitza = "";
				if(question==null)
					label.setText("You need to choose a Question");
				else
				{
				if(!question.getFees().isEmpty())
				{
				for(domain.Fee f:question.getFees())
					if(f.getResult()!=null)
					emaitza+= facade.getFee(f.getId()) + '\n';
		...
		btnSetResult.setBounds(179, 478, 130, 30);
		...
		label.setBounds(179, 410, 299, 37);
		getContentPane().add(label);
		
		JLabel lblFees = new JLabel(ResourceBundle.getBundle("Etiquetas").getString("CheckFeesGUI.lblFees.text")); //$NON-NLS-1$ //$NON-NLS-2$
		lblFees.setBounds(377, 218, 75, 14);
		getContentPane().add(lblFees);
		textArea.setText(ResourceBundle.getBundle("Etiquetas").getString("CheckFeesGUI.textArea.text")); //$NON-NLS-1$ //$NON-NLS-2$
		textArea.setBounds(361, 243, 277, 135);
		
		getContentPane().add(textArea);
		
		JButton button = new JButton(ResourceBundle.getBundle("Etiquetas").getString("CheckFeesGUI.button.text")); //$NON-NLS-1$ //$NON-NLS-2$
		button.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				if(CheckFeesGUI.getErabiltzaile() instanceof Registered) {
					leihoa.setVisible(false);
					RegisteredGUI a = new RegisteredGUI(CheckFeesGUI.getErabiltzaile());
					a.setVisible(true);
					a.setErabiltzaile(CheckFeesGUI.getErabiltzaile());
				}
				else if(CheckFeesGUI.getErabiltzaile() instanceof Admin){
					leihoa.setVisible(false);
					AdminGUI a = new AdminGUI();
					a.setVisible(true);
					a.setErabiltzaile(CheckFeesGUI.getErabiltzaile());
				}
				else if(CheckFeesGUI.getErabiltzaile() instanceof Worker){
					leihoa.setVisible(false);
		...
		button.setBounds(398, 482, 89, 23);
	...
	public static void setErabiltzaile(User erabiltzaile) {
		CheckFeesGUI.erabiltzaile = erabiltzaile;
	}
}

...
public class DeleteFeeGUI extends JFrame {
			...
			public void actionPerformed(ActionEvent e) {
				Date data = trim(firstDay);
	  		    if (data.before(Calendar.getInstance().getTime())) {
					...
					label.setText("You need to select a Fee");
				else 
				{

					BLFacade facade=AdminGUI.getBusinessLogic();
					facade.deleteFees(fee.getId());
					comboBox.removeItem(fee);
					JOptionPane.showMessageDialog(null, "The Fee Was Succesfully Removed");
	...
	public static User getErabiltzaile() {
		return DeleteFeeGUI.erabiltzaile;
	}
